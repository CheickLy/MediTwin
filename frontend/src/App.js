...
// --- 3D MODEL COMPONENT ---
const PatientModel = ({ peakValue, substance }) => {
  const threshold = substance === 'alcohol' ? 8.0 : 1.0;
  const isDanger = peakValue > threshold;

  return (
    <Float speed={2} rotationIntensity={0.5} floatIntensity={0.5}>
      <Sphere args={[1, 100, 100]} scale={1.4}>
        <MeshDistortMaterial
          color={isDanger ? "#ff2d2d" : "#00d2ff"}
          speed={isDanger ? 8 : 2}
          distort={isDanger ? 0.5 : 0.3}
          roughness={0.2}
          metalness={0.8}
        />
      </Sphere>
...
export default function App() {
  ...
  const [data, setData] = useState({ levels: [], peak_value: 0, hours_to_safe: 0 });
  ...
  useEffect(() => { fetchData(); }, [weight, genFactor, substance]);

  const isToxic = (substance === 'pill' ? data.peak_value > 1.0 : data.peak_value > 8.0);

  return (
    <div style={{ display: 'flex', height: '100vh', background: '#0a0a0c', color: 'white', fontFamily: 'sans-serif' }}>
      
      {/* SIDEBAR */}
      <div style={{ width: '350px', padding: '40px', borderRight: '1px solid #333', background: '#111' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <Canvas camera={{ position: [0, 0, 4] }}>
          ...
          <PatientModel peakValue={data.peak_value} substance={substance} />
          <OrbitControls enableZoom={false} />
        </Canvas>

        {/* RIGHT SIDE INDICATORS */}
        <div style={{ position: 'absolute', top: '40px', right: '40px', textAlign: 'right' }}>
           <div style={{ background: 'rgba(0,0,0,0.5)', padding: '20px', borderRadius: '12px', border: '1px solid #222' }}>
              <p style={{ color: '#888', margin: 0, fontSize: '12px' }}>ESTIMATED RECOVERY</p>
              <h2 style={{ fontSize: '32px', margin: '5px 0', color: isToxic ? '#ff2d2d' : '#00d2ff' }}>
                {isToxic ? `${data.hours_to_safe.toFixed(1)} hrs` : "Stable"}
              </h2>
        ...
        <div style={{ position: 'absolute', bottom: '40px', right: '40px', textAlign: 'right' }}>
          ...
          <h1 style={{ fontSize: '56px', margin: 0, color: isToxic ? '#ff2d2d' : '#00d2ff' }}>
            {data.peak_value.toFixed(2)} <span style={{ fontSize: '20px' }}>mg/L</span>
          </h1>
        </div>
      </div>
    </div>
